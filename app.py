...
CANCHA_JSON = None
try:
    with open("cancha_futsal.json", encoding="utf-8") as _f:
...
@st.cache_data
def cargar_equipos():
    with open(BASE / "equipos.json", encoding="utf-8-sig") as f:
        return json.load(f)["equipos"]

@st.cache_data
def cargar_equipo(equipo_id):
    with open(BASE / equipo_id / "equipo.json", encoding="utf-8-sig") as f:
        return json.load(f)
...
def cargar_partidos(equipo_id):
    ...
    for p in sorted(carpeta.iterdir(), reverse=True):
        if p.is_dir() and (p / "partido.json").exists():
            with open(p / "partido.json", encoding="utf-8-sig") as f:
                data = json.load(f)
                data["_carpeta"] = str(p)
...
# ════════════════════════════════════════════════════════
with st.sidebar:
    logo_path = Path("logo_circular.png")
